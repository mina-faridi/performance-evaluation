...
global EVENT_ARRIVAL EVENT_END_CPU_RUN  EVENT_END_SIMULATION LIST_CPU LIST_QUEUE  SAMPST_RESPONSE_TIMES STREAM_THINK STREAM_SERVICE ;
 ...
 LIST_QUEUE= 1 ;%  /* List number for CPU queue. */
 LIST_CPU= 2 ;%  /* List number for CPU. */
...
min_terms = read_data_in(1);
max_terms  = read_data_in(2);
incr_terms  = read_data_in(3);
num_responses_required = read_data_in(4);
mean_think = read_data_in(5);
mean_service = read_data_in(6);
quantum = read_data_in(7);
swap = read_data_in(8);
...
function  end_CPU_run
    ...
    if (transfer(2) > 0.0)  
        ...
        event_schedule(sim_time + expon(mean_think, STREAM_THINK),EVENT_ARRIVAL);
...
function  report
    ...
    fprintf(outfile, "\n\n%5d%16.3f%16.3f%16.3f", num_terms,sampst(0.0, -SAMPST_RESPONSE_TIMES), filest(LIST_QUEUE),filest(LIST_CPU));
    
    
end

function  start_CPU_run
...
while(min_terms <= max_terms)
    
    num_terms = num_terms + incr_terms ;
    % /* Initialize simlib */
